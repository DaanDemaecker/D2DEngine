...
namespace D2D
{
	void LoadBombermanScene(Scene& scene)
	{
		...
		pGameUIComponent->Initialize(pIntroScreen, pHud, pWorld, pTimerComponent.get(), pPointsDisplayComponent.get(), pLivesDisplayComponent.get(), pTimerComponent.get(), pGameOverScreen, pLevelEndScreen, pIntroScreenText.get());
